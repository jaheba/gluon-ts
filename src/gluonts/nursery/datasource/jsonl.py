...
class JsonLinesFiles:
    def __init__(self, path: Path):
        self.paths = list(path.glob("*"))
        self.handlers = list(map(JsonLinesFile, self.paths))
    ...
    def __len__(self):
        return sum(map(len, self.handlers))


FileDataset = JsonLinesFiles
...
def partition_file(xs: JsonLinesFile, n):
...
@glide.partition.register
def partition_files(xs: JsonLinesFiles, n):
    # TODO: find a way to end up with n-handlers
    if len(xs.handlers) == 1:
        return glide.partition(xs.handlers[0], n)
